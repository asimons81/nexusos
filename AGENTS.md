...
**Phase 0 + Phase 1 complete; Phase 2 kernel + CLI core done; v0.1.0 feature-complete.** The internal `indexing/` kernel is implemented (deterministic workspace-scoped IDs, SQLite schema v1 with FTS5, explicit migrations, transactional persistence, exclusive-writer lock, `IndexKernel` API) and the CLI exposes `index`, `status`, `search`, `browse`, `read`, `recent`, `links`, `context`, `lint`, `serve`, `mcp`, and `demo`. Search is SQLite FTS5 only (bm25-ranked, prefix matching, line-aware excerpts, read-only). An MCP server (`nexusos mcp` / `python -m nexusos.mcp`, stdio transport; `nexusos serve --transport streamable-http`, loopback-only HTTP) exposes status, search, browse, read, recent, links, context, and index as MCP tools with strict schemas; it reuses the read-only services for retrieval and the index service for corpus indexing. `nexusos lint --workspace` is the read-only workspace vault linter (broken links, orphans, duplicate slugs, stale index, oversized/empty docs, symlink escapes, files outside collections); plain `nexusos lint` remains kernel dev tooling. Embeddings, connectors, cloud, source mutation, and non-loopback transports are not yet implemented. Do not add placeholder commands that falsely claim these features work.
